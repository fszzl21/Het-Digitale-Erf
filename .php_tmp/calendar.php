...
if ($_SERVER["REQUEST_METHOD"] === "POST" && ($_POST["action"] ?? "") === "add") {
  ...
  if ($appointment && $theme && $start && $end) {
    $stmt = $conn->prepare(
      "INSERT INTO appointments (appointment_name, appointment_theme, start_date, end_date, start_time, end_time) VALUES (?, ?, ?, ?, ?, ?)"
    );
...
# Handle Edit Appointment
 if ($_SERVER["REQUEST_METHOD"] === "POST" && ($_POST["action"] ?? "") === "edit") {
  ...
  if ($id && $appointment && $theme && $start && $end) {
    $stmt = $conn->prepare(
      "UPDATE appointments SET appointment_name = ?, appointment_theme = ?, start_date = ?, end_date = ?, start_time = ?, end_time = ? WHERE id = ?"
    );
