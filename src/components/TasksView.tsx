  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-gray-900">Taken Beheer</h2>
        <button
      ...
      {showAddForm && (
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h3 className="text-gray-900 mb-4">Nieuwe Taak Toevoegen</h3>
          <div className="space-y-4">
            <div>
              <label className="block text-sm text-gray-700 mb-2">Titel</label>
              <input
                type="text"
                value={newTask.title}
            ...
            <div>
              <label className="block text-sm text-gray-700 mb-2">Beschrijving</label>
              <textarea
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm text-gray-700 mb-2">Prioriteit</label>
                <select
              ...
              <div>
                <label className="block text-sm text-gray-700 mb-2">Toegewezen aan</label>
                <input
                  type="text"
      ...
      <div className="space-y-4">
        {tasks.map((task) => (
          <div
            key={task.id}
            className={`bg-white p-6 rounded-xl shadow-sm border border-gray-100 transition-all ${task.status === 'done' ? 'opacity-60' : ''
              }`}
          >
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <div className="flex items-start justify-between mb-2">
                  <h3 className={`text-gray-900 ${task.status === 'done' ? 'line-through' : ''}`}>
                    {task.title}
                  </h3>
                  <button
                ...
                <div className="flex items-center gap-3">
                  ...
                  {task.assigned_to && (
                    <span className="text-sm text-gray-600">
                      Toegewezen aan: <span className="text-gray-900">{task.assigned_to}</span>
                    </span>
                  )}
                </div>
              </div>
        ...
        {tasks.length === 0 && (
          <div className="bg-white p-12 rounded-xl shadow-sm border border-gray-100 text-center">
            <p className="text-gray-500">Geen taken gevonden. Klik op "Nieuwe Taak" om te beginnen.</p>
          </div>
        )}
      </div>
    </div>
