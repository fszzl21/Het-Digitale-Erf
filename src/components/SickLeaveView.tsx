  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-gray-900">Ziekte & Verzuim Beheer</h2>
        <button
      ...
      {showAddForm && (
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h3 className="text-gray-900 mb-4">Nieuwe Ziekmelding</h3>
          <div className="space-y-4">
            <div>
              <label className="block text-sm text-gray-700 mb-2">Medewerker</label>
              <input
                type="text"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm text-gray-700 mb-2">Startdatum</label>
                <input
                  type="date"
              ...
              <div>
                <label className="block text-sm text-gray-700 mb-2">Verwachte einddatum (optioneel)</label>
                <input
                  type="date"
            ...
            <div>
              <label className="block text-sm text-gray-700 mb-2">Reden (optioneel)</label>
              <textarea
      ...
      <div>
        ...
        <div className="space-y-4">
          {activeSickLeaves.map((sickLeave) => (
            <div key={sickLeave.id} className="bg-white p-6 rounded-xl shadow-sm border-l-4 border-l-red-500 border-t border-r border-b border-gray-100">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  <User className="w-5 h-5 text-gray-600" />
                  <h4 className="text-gray-900">{sickLeave.employee}</h4>
                </div>
              ...
              <div className="space-y-2">
                ...
                {sickLeave.reason && (
                  <p className="text-sm text-gray-600">Reden: {sickLeave.reason}</p>
                )}
              </div>
            </div>
          ))}
      ...
      {/* Past Sick Leaves */}
      <div>
        <h3 className="text-gray-900 mb-4">Verlopen Ziekmeldingen</h3>
        <div className="space-y-4">
          {pastSickLeaves.map((sickLeave) => (
            <div key={sickLeave.id} className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 opacity-60">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  <User className="w-5 h-5 text-gray-600" />
                  <h4 className="text-gray-900">{sickLeave.employee}</h4>
          ...
          {pastSickLeaves.length === 0 && (
            <div className="bg-white p-8 rounded-xl shadow-sm border border-gray-100 text-center">
              <p className="text-gray-500">Geen verlopen ziekmeldingen</p>
            </div>
          )}
        </div>
      </div>
