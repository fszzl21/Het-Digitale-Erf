  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-background">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Role Badge */}
        <div className="mb-6">
          <div className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-card rounded-lg border border-gray-200 dark:border-border shadow-sm w-fit transition-colors">
            <Shield className="w-4 h-4 text-green-700 dark:text-green-400" />
            <span className="text-sm text-gray-600 dark:text-muted-foreground">Rol:</span>
            <span className="text-sm text-gray-900 dark:text-foreground">{user?.role}</span>
          </div>
        </div>

        {/* Navigation Tabs */}
        <div className="flex gap-4 mb-8 border-b border-gray-200 dark:border-border overflow-x-auto">
          <button
            onClick={() => setActiveTab('overview')}
            className={`pb-4 px-2 whitespace-nowrap transition-colors ${activeTab === 'overview'
              ? 'border-b-2 border-green-700 dark:border-green-400 text-green-700 dark:text-green-400'
